...
final class NioTcpClient extends NioClient {
    ...
    static {
        addSelectorTimeoutTask(new Runnable() { // from class: org.xbill.DNS.NioTcpClient$$ExternalSyntheticLambda3
            @Override // java.lang.Runnable
            public final void run() {
                NioTcpClient.processPendingRegistrations();
            }
        });
        addSelectorTimeoutTask(new Runnable() { // from class: org.xbill.DNS.NioTcpClient$$ExternalSyntheticLambda4
            @Override // java.lang.Runnable
            public final void run() {
                NioTcpClient.checkTransactionTimeouts();
            }
        });
        addCloseTask(new Runnable() { // from class: org.xbill.DNS.NioTcpClient$$ExternalSyntheticLambda5
            @Override // java.lang.Runnable
            public final void run() {
    ...
    public static void processPendingRegistrations() {
        while (true) {
            ...
            ChannelState channelStateRemove = queue.remove();
            try {
                Selector selector = selector();
                if (!channelStateRemove.channel.isConnected()) {
                    channelStateRemove.channel.register(selector, 8, channelStateRemove);
                } else {
                    channelStateRemove.channel.keyFor(selector).interestOps(4);
                }
            } catch (IOException e) {
                channelStateRemove.handleChannelException(e);
            }
        }
    }
    ...
    public static void closeTcp() {
        ...
        Map.EL.forEach(map, new BiConsumer() { // from class: org.xbill.DNS.NioTcpClient$$ExternalSyntheticLambda1
    ...
    private static class Transaction {
        ...
        void send() throws IOException {
            ...
            ByteBuffer byteBufferAllocate = ByteBuffer.allocate(this.queryData.length + 2);
            byteBufferAllocate.put((byte) (this.queryData.length >>> 8));
            byteBufferAllocate.put((byte) (this.queryData.length & 255));
            byteBufferAllocate.put(this.queryData);
            byteBufferAllocate.flip();
            while (byteBufferAllocate.hasRemaining()) {
                if (this.channel.write(byteBufferAllocate) < 0) {
    ...
    static CompletableFuture<byte[]> sendrecv(final InetSocketAddress inetSocketAddress, final InetSocketAddress inetSocketAddress2, Message message, byte[] bArr, Duration duration) {
        final CompletableFuture<byte[]> completableFutureM2226m = NioSystemFileSystem$$ExternalSyntheticApiModelOutline0.m2226m();
        try {
            Selector selector = selector();
            long jNanoTime = System.nanoTime() + duration.toNanos();
            ChannelState channelState = (ChannelState) Map.EL.computeIfAbsent(channelMap, new ChannelKey(inetSocketAddress, inetSocketAddress2), new Function() { // from class: org.xbill.DNS.NioTcpClient$$ExternalSyntheticLambda2
                ...
                public final Object apply(Object obj) {
                    return NioTcpClient.lambda$sendrecv$1(inetSocketAddress, inetSocketAddress2, completableFutureM2226m, (NioTcpClient.ChannelKey) obj);
                }
            ...
            if (channelState != null) {
                ...
                channelState.pendingTransactions.add(new Transaction(message, bArr, jNanoTime, channelState.channel, completableFutureM2226m));
    ...
    static /* synthetic */ ChannelState lambda$sendrecv$1(InetSocketAddress inetSocketAddress, InetSocketAddress inetSocketAddress2, CompletableFuture completableFuture, ChannelKey channelKey) {
        try {
